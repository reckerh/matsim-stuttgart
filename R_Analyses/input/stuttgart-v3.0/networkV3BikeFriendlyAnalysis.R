...
##### Read data #####
t.dat <- read.csv("networkAttributesStuttgartBikeFriendly.csv")
dat <- t.dat
...
head(dat)
rm(res, i, temp)
#st_write(subset(dat, select = -c(allowedModesVector, fromPoint, toPoint)), "networkAttributesStuttgartV3BikeFriendly.geojson")
...
ggsave(filename = "plot_bikeInfra_v3BikeFriendly.png", height = 8, width = 8)
...
), path = "distributionAnalysis_scoringAttributes_stuttgartV3BikeFriendly.xlsx")
...
### analyse newly added links ###

#extract new links (can be determined by Id suffix '_cyleway')
newLinks <- datStutt[which(endsWith(datStutt$linkId, "_cycleway")),]
table(dat$linkId[which(endsWith(dat$linkId, "_cycleway"))] == newLinks$linkId) #only links within in Stuttgart have new suffix
#also, the number of new links is the same as the number of links (in Stuttgart) that should be selected in the scenario

#confirm that only links in stuttgart are newly created by comparing suffixes
#plot new links extracted from links in stuttgart
ggplot() +
  geom_sf(data = stuttgart) +
  geom_sf(data = newLinks)
#plot new links extracted from all links
ggplot() +
  geom_sf(data = stuttgart) +
  geom_sf(data = dat[which(endsWith(dat$linkId, "_cycleway")),])

#look at distribution of lengths of infra types
aggregate(length ~ infraString, data = newLinks, FUN = sum)

#calculate possible kilometers driven per hour on the new links (-> mögliche Fahrleistung)
units::drop_units(sum(newLinks$length * newLinks$capacity)) / 1000



### plot new / edited links vs old links ###
ggplot() +
  geom_sf(data = stuttgart, aes(fill = "Stuttgart"), show.legend = FALSE) +
  geom_sf(data = datStutt, aes(colour = "Kapazität nicht reduziert")) +
  geom_sf(data = datStutt[which(endsWith(datStutt$linkId, "_cycleway")),], aes(colour = "Kapazität reduziert")) +
  theme_bw() +
  theme(axis.ticks = element_blank(), axis.text.x = element_blank(), axis.text.y = element_blank(),
        panel.grid = element_blank(), panel.border = element_blank(), legend.position = "bottom") +
  scale_color_manual(name = "Legende",
                     breaks = c("Kapazität nicht reduziert", "Kapazität reduziert"),
                     values = c("Kapazität nicht reduziert" = "black",
                                "Kapazität reduziert" = "blue")) +
  scale_fill_manual(name = "Stuttgart",
                    breaks = c("Stuttgart"),
                    values = c("Stuttgart" = "white"))
ggsave(filename = "plot_editedLinks_v3BikeFriendly.png", height = 8, width = 8)
