CREATE MATERIALIZED VIEW matsim_output.distance_groups AS
	...
	groups AS
		...
		GROUP BY distance_group, main_mode)
		
	SELECT
		*,  ROUND((no_trips / SUM(no_trips) OVER (partition by main_mode))* 100, 1) AS mode_share
	FROM groups
	ORDER BY main_mode, distance_group
